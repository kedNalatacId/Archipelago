...
class DefaultProgression(BaseProgression):
    def _region_access_rule(
        ...
        if side == CelesteSide.A_SIDE:
            ...
            # If no cassettes are in the game, then B-Sides aren't accessible, so just lie and say B-Sides are open
            if not self.get_option("include_cassettes"):
    ...
    def _location_access_rule(
        ...
        if level == self._goal_level() and side == self._goal_side():
            hr = self.get_option("hearts_required")
            sr = self.get_option("berries_required")
            lr = self.get_option("levels_required")
            cr = self.get_option("cassettes_required")
# WIP
#           print(f"Checking for access to goal level; hearts: {hr} / straw: {sr} / levels: {lr} / cass: {cr}")
# END WIP
            prev = level.previous()
            return lambda state: (
                state.has_group(
                ...
                and (self.get_option("cassettes_required") == 0 or state.has_group(
                    "cassettes",
                    player,
    ...
    def items(self, player: int, multiworld: MultiWorld) -> List[CelesteItem]:
        ...
        if not self._options["include_cassettes"]:
            self._options["cassettes_required"] = 0
        else:
        ...
        for item_type, level, side, name, uuid in BaseData.items(self._goal_level(), self._goal_side()):
            ...
            if not self.get_option("include_bsides") and (side == CelesteSide.B_SIDE or side == CelesteSide.C_SIDE):
                continue
            if not self.get_option("include_csides") and side == CelesteSide.C_SIDE:
                continue
            if not self.get_option("include_cassettes") and item_type == CelesteItemType.CASSETTE:
                continue
    ...
    def locations(self, player: int, multiworld: MultiWorld) -> Dict[int, CelesteLocation]:
        ...
        for level, side, name, uuid in BaseData.locations(self._goal_level(), self._goal_side()):
            ...
            if not self.get_option("include_csides") and side == CelesteSide.C_SIDE:
                continue
            if not self.get_option("include_cassettes") and "Cassette" in name:
                continue
