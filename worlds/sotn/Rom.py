...
USHASH1 = 'acbb3a2e4a8f865f363dc06df147afa2'
USHASH2 = '8f4b1df20c0173f7c2e6a30bd3109ac8'
...
# Check Track 2 while we're here (jic)
def get_base_rom_bytes() -> bytes:
    with open(get_settings().sotn_settings.track_2_rom_file, "rb") as infile:
        rom_track_2_bytes = bytes(infile.read())
    md5_track_2 = hashlib.md5()
    md5_track_2.update(rom_track_2_bytes)
    if USHASH2 != md5_track_2.hexdigest():
        raise Exception('Supplied Base Rom does not match known MD5 for US0067 release Track 2. '
                        'Get the correct game and version, then dump it')

    with open(get_settings().sotn_settings.track_1_rom_file, "rb") as infile:
        base_rom_bytes = bytes(infile.read())

    md5_track_1 = hashlib.md5()
    md5_track_1.update(base_rom_bytes)
    if USHASH1 != md5_track_1.hexdigest():
        raise Exception('Supplied Base Rom does not match known MD5 for SLU067 release Track 1. '
                        'Get the correct game and version, then dump it')
...
def get_base_rom_path(file_name: str = "") -> str:
    ...
    if not file_name:
        file_name = options["sotn_options"]["track_1_rom_file"]
...
def patch_rom(world: World, output_dir: str) -> str:
    player = world.player
    ...
    bosses = world.options.bosses_needed
    ...
    # This is currently the entire file (not the bsdiff of the file)
    # Do NOT include it in the temp dir like other output (yet)
    # filename = f'{output_dir}/SOTN_{seed_num}_P{player}.bin'
...
def write_to_file(buffer, filename=""):
    ...
    track2 = get_settings().sotn_settings.track_2_rom_file
    if filename == "":
        output_file = get_settings().sotn_settings.track_1_rom_file

    cue_file = f'FILE "{filename}" BINARY\n  TRACK 01 MODE2/2352\n\tINDEX 01 00:00:00\n'
    cue_file += f'FILE "{track2}" BINARY\n  TRACK 02 AUDIO\n'
    cue_file += '\tINDEX 00 00:00:00\n\tINDEX 01 00:02:00'
