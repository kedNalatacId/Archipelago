...
ROMHASH   = "acbb3a2e4a8f865f363dc06df147afa2"
...
def get_base_rom_bytes() -> bytes:
    ...
    if ROMHASH != md5_track_1.hexdigest():
        raise Exception('Supplied Track 1 Base Rom does not match known MD5 for SLU067 release Track 1. '
                        'Get the correct game and version, then dump it')

    md5_track_2 = hashlib.md5()
    md5_track_2.update(audio_rom_bytes)
    if AUDIOHASH != md5_track_2.hexdigest():
        raise Exception('Supplied Track 2 Audio Rom does not match known MD5 for SLU067 release Track 2. '
                        'Get the correct game and version, then dump it')
...
def patch_rom(world: World, output_directory: str) -> None:
    ...
    exp = world.options.exploration_needed
    if bosses > 20:
        bosses = 20
    if exp > 20:
        exp = 20
...
def randomize_prices(buffer, prices):
    min_prices = 1 * 10**prices
    max_prices = 10 * 10**prices
