        ...
        if cmd == 'Connected':
            ...
            if 'item' in args and args['type'] == "ItemSend" and args['receiving'] == args['item']['player']:
                ...
                if received.location == 127083080 or received.location == 127020003:
                    ...
                    self.items_received.append(166)
...
def convert_item_id(ap_item_id: int):
    return ap_item_id - base_item_id
...
def get_payload(ctx: SotnContext):
    ...
    for item in ctx.items_received:
        import pprint
#       print(f"Attempting to receive item (type: {type(item)}:")
#       print(f"item has attr 'item': {hasattr(item, 'item')}")
#       pprint.pprint(item)
        if hasattr(item, "item"):
#           print("Received an Item! Appending to items list.")
